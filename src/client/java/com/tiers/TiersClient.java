...
public class TiersClient implements ClientModInitializer {
    ...
    public static void tiersCommand(String playerName) {
        if (playerName.equalsIgnoreCase("-toggle"))
            ...
            sendMessageToPlayer(Text.literal("- Report a bug: ").append(Text.literal("Tiers GitHub issues").styled(style -> style.withUnderline(true).withClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, "https://github.com/PvPTiers/Tiers/issues")))), false);
            sendMessageToPlayer(Text.literal("- It's not advisable to create tickets in PvPTiers support"), false);
            sendMessageToPlayer(Text.literal("- ").append(Text.literal("Changelogs").styled(style -> style.withUnderline(true).withClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, "https://github.com/PvPTiers/Tiers/wiki/Version-changelogs")))), false);
    ...
    public static void updateCaches() {
        ...
        for (Entity entity : minecraftClient.world.getEntities())
            if (entity instanceof DisplayEntity.TextDisplayEntity textDisplay)
                ((DataTrackerAccessor) textDisplay.getDataTracker()).invokeSet(TextDisplayAccessor.getTEXT(), ((TextDisplayEntityInvokerClientMixin) entity).invokeGetText(), true);
    }
